...
*Пояснение:* Синтаксис fish кардинально отличается от `bash/zsh`: при объявлении алиасов и переменных знак `=` не используется. Переменные объявляются через `set -x`.
...
*Пояснение:* Конвейер выводит список всех процессов, фильтрует по текущему пользователю и считает строки. Итоговое число на 1 больше , так как сам процесс `grep` тоже попадает в этот список.
...
*Пояснение:* Временные алиасы удаляются после закрытия сессии терминала. Постоянные алиасы были зафиксированы в `~/.bashrc`, `~/.zshrc` и `config.fish` и применены через команду `source`
...
*Пояснение:* Локальная TEMP_VAR оказалась недоступна во вложенном процессе bash, тогда как GLOBAL_VAR, объявленная через `export` отобразилась. Переменные EDITOR и WORKSPACE сделаны постоянными.

#### Раздел 2.9. Комплексный скрипт
Создан скрипт резервного копирования `lab2_script.sh`

![Команды](2.9/2.9.png)
![Команды](2.9/2.9.1.png)

*Пояснение:* Скрипт принимает аргумент, проверяет его наличие (при отсутствии выводит ошибку в stderr и завершается с кодом 1), копирует файл с .bak и пишет лог с датой.
...
С помощью `|`. Пример: `ls -l | grep "data"`
