...
*Пояснение:* Локальная переменная TEMP_VAR оказалась недоступна во вложенном процессе bash, тогда как GLOBAL_VAR, объявленная через `export`, успешно отобразилась. Переменные EDITOR и WORKSPACE сделаны постоянными.
...
*Пояснение:* Скрипт принимает аргумент, проверяет его наличие (при отсутствии выводит ошибку в stderr и завершается с кодом 1), копирует файл с суффиксом .bak и пишет лог с датой.
